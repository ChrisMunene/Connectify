...
app.config["SESSION_FILE_DIR"] = os.getcwd()
...
def home():
    ...
    rows = db.execute("DELETE FROM songs WHERE userid = :id AND ctid NOT IN (SELECT min(ctid) FROM songs WHERE userid = :id GROUP BY name, artist)", id=session["user_id"])
